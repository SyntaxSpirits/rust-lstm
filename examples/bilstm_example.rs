...
fn generate_bidirectional_data() -> Vec<Array2<f64>> {
    ...
    for t in 0..sequence_length {
        ...
        let current = t_f.sin();
        let future = if t < sequence_length - 1 {
            (t_f + 0.5).cos() * 0.5
        } else {
            0.0
        };
...
fn demo_bilstm_vs_lstm() {
    ...
    println!(
        "  Sample output: [{:.4}, {:.4}, {:.4}, {:.4}]",
        bilstm_outputs[0][[0, 0]],
        bilstm_outputs[0][[1, 0]],
        bilstm_outputs[0][[2, 0]],
        bilstm_outputs[0][[3, 0]]
    );
