...
use rust_lstm::{Adam, GRULayerDropoutConfig, GRUNetwork, LossFunction, MSELoss};
...
fn multilayer_gru_example() {
    ...
    let mut gru =
        GRUNetwork::new(input_size, hidden_size, num_layers).with_layer_dropout(layer_configs);
    ...
    println!(
        "   Network: {} layers, {} input size, {} hidden size",
        num_layers, input_size, hidden_size
    );
    println!(
        "   Training mode outputs shape: {:?}",
        outputs_train.last().unwrap().shape()
    );
    println!(
        "   Evaluation mode outputs shape: {:?}",
        outputs_eval.last().unwrap().shape()
    );
...
fn sequence_modeling_example() {
    ...
    for (i, (output, _layer_outputs)) in outputs.iter().enumerate() {
        println!(
            "   Step {} output norm: {:.4}",
            i,
            output.iter().map(|&x| x * x).sum::<f64>().sqrt()
        );
    }
}
...
fn simple_training_example() {
    ...
    for epoch in 0..5 {
        ...
        println!(
            ...
            epoch + 1,
            total_loss / train_sequences.len() as f64
        );
    }
