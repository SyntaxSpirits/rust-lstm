...
fn main() {
    ...
    println!(
        "  BiLSTM output size: {} (concat mode)",
        bilstm.output_size()
    );
    ...
    for (text, expected) in &test_sentences {
        ...
        let bilstm_pred = if bilstm_sentiment > 0.1 {
            "positive"
        } else if bilstm_sentiment < -0.1 {
            "negative"
        } else {
            "mixed"
        };
        let lstm_pred = if lstm_sentiment > 0.1 {
            "positive"
        } else if lstm_sentiment < -0.1 {
            "negative"
        } else {
            "mixed"
        };
        ...
        let bilstm_correct =
            bilstm_pred == *expected || (*expected == "mixed" && bilstm_sentiment.abs() < 0.2);
        let lstm_correct =
            lstm_pred == *expected || (*expected == "mixed" && lstm_sentiment.abs() < 0.2);

        println!(
            "  BiLSTM correct: {}",
            if bilstm_correct { "✓" } else { "✗" }
        );
