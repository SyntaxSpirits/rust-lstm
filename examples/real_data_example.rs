...
fn main() {
    ...
    for i in 24..29 {
        ...
        if let Some(predicted_temp) = predictor.predict_next(&data_loader, input_data) {
            ...
            println!(
                "Hour {}: Predicted={:.1}°C, Actual={:.1}°C, Error={:.1}°C",
                i + 1,
                predicted_temp,
                actual_temp,
                error
    ...
    let temp_range = temps
        .iter()
        .fold((f64::INFINITY, f64::NEG_INFINITY), |(min, max), &t| {
            (min.min(t), max.max(t))
        });
